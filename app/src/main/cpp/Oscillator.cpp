            ...
            for(int k=0; k<MAX_POLYPHONY; k++){
                if(notes_[k] >= NOTE_C_0){
                    /*for(int j=0; j < HARMONICS; j++) {
                        audioData[i] += (float) (sin((phase_[k] * pow(2,j)) /2 ) * AMPLITUDE *
                    ...
                    temp += (float) (sin((phase_[k])) *2.0 ) * AMPLITUDE;
                    temp += (float) (sin((phase_[k]))   ) * decibel2linear(-4.0) * AMPLITUDE;
                    //then multiplied by another sine, also known as amplitude modulation (AM):
                    temp = temp *( (float) sin((phase_[k]))  * decibel2linear(-4.0) );
                    */

                    //another sound I stumbled upon while writing this code, idk how, but it sounds like a sawtooth
                    audioData[i] += (float) sin((phase_[k]) /2 ) * AMPLITUDE;
                    audioData[i] += (float) sin((phase_[k]) /4 ) * AMPLITUDE * decibel2linear(-4);
